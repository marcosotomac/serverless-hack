...
def export_to_excel(incidents):
    ...
    try:
        ...
        raise Exception("openpyxl no está instalado. Instale con: pip install openpyxl")
...
def export_to_pdf(incidents):
    ...
    if incidents:
        elements.append(Paragraph("Detalles de Incidentes", styles['Heading2']))
...
def handler(event, context):
    ...
    try:
        ...
        if filters:
            ...
            if "type" in filters:
                incidents = [i for i in incidents if i.get("type") == filters["type"]]
            if "urgency" in filters:
                incidents = [i for i in incidents if i.get("urgency") == filters["urgency"]]
